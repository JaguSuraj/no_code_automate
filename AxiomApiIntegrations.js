...
export class AxiomApiIntegrations {
    ...
    static createSheetInGoogleSheet(params, callback) {
        this.getSpreadsheetIdFromUrl(params.spreadsheet_id, async spreadsheet_id => {
            ...
            await user.load()
            apiProvider.sendGqlMutation(
                `mutation CreateWorkSheet($spreadsheet_id: String!, $title: String!, $user_id: Int! ) {  
                    CreateWorkSheet(
                ...
                {spreadsheet_id: spreadsheet_id, title: params.title, user_id: res}
            ).then(res2 => {
                ...
                if (err.error.status) {
                    switch (err.error.status){
                        case 'PERMISSION_DENIED':
                            callback({ error: "You do not have permissions to access this Google sheet, you will need to share the sheet with the Google account that you have connected to your Axiom account."})
                            break
                        case 'NOT_FOUND':
